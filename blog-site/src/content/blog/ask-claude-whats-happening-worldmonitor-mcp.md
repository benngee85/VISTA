...
description: "Connect Claude to VISTA's MCP server so non-developers can ask live questions about conflicts, markets, chokepoints, country risk, and breaking news."
metaTitle: "Ask Claude About Current Events with VISTA MCP"
keywords: "Claude connectors real-time news, ask AI about current events, Claude MCP server current events, VISTA MCP Claude, AI current events assistant"
...
VISTA's [MCP server](https://www.worldmonitor.app/docs/mcp-overview) gives Claude a live tool connection. You add one connector URL, approve the OAuth flow, and Claude can call the same intelligence surfaces behind the [VISTA dashboard](/blog/posts/what-is-worldmonitor-real-time-global-intelligence/): country risk, conflicts, markets, maritime chokepoints, aviation, cyber, energy, news intelligence, forecasts, and more.

You do not need to write an app. You do not need to understand JSON-RPC. You only need a paid VISTA tier and a Claude client that supports custom connectors.
...
VISTA's MCP server exists for this exact pattern. It exposes 59 tools, six prompt templates, and read-only resources through the [Model Context Protocol](https://modelcontextprotocol.io), so Claude can discover tools and call them during a conversation.
...
For Claude web, open Settings, go to Connectors, add a custom connector named `VISTA`, and paste that URL.
...
Restart Claude Desktop after saving the file. On first use, Claude opens the VISTA OAuth consent flow. Pro users sign in with their VISTA account. API Starter, Business, and Enterprise users can use OAuth or an issued `wm_...` API key for server-side workflows.
...
You can ask plain-English questions, but the fastest way to get reliable answers is to start from repeatable prompt shapes. These are the six templates VISTA exposes through MCP:
...
```text
Use VISTA before answering. Give me a 10-minute global brief for today.
...
If Claude answers without calling tools, ask it to retry with VISTA tools first.
...
```text
Use VISTA to brief Turkey for an operations team. Include country risk,
...
```text
Use VISTA to check route risk from the Gulf to Northwest Europe. Look at
...
```text
Use VISTA to evaluate this claim: "<paste claim>". Return an evidence
...
```text
Use VISTA for a market-open geopolitical prep. Include oil, gold, major
...
```text
Run a VISTA freshness audit for the tools you just used. Which fields are
...
- Do not assume VISTA covers every source on earth.
...
**Can non-developers use VISTA MCP with Claude?**

Yes. Claude web users can add VISTA as a custom connector, and Claude Desktop users can add the server URL to the desktop config. The setup is mostly copy-paste; no application code is required.

**Does this give Claude real-time current events?**

It gives Claude access to VISTA's live and cache-backed intelligence tools. Claude still has to call the tools, preserve freshness fields, and explain uncertainty. Ask it to use VISTA before answering live-risk questions.

**Is MCP free?**

The public dashboard is free, but VISTA MCP access requires Pro or an API tier. The MCP docs list the current auth modes, quotas, and client setup details.
