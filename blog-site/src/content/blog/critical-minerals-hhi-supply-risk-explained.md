...
description: "How VISTA uses 2024 production data and the Herfindahl-Hirschman Index to explain supply concentration in lithium, cobalt, rare earths, and more."
metaTitle: "Critical Minerals HHI Supply Risk Explained | VISTA"
...
VISTA's Critical Minerals view uses the Herfindahl-Hirschman Index, or HHI, to make that concentration visible. The current dataset uses 2024 global production entries for minerals such as lithium, cobalt, rare earths, gallium, and germanium, then computes concentration and risk labels from producer shares.
...
VISTA's supply-chain scoring code labels HHI like this:
...
For each mineral, VISTA groups production by mineral, sums producer tonnes, computes producer shares, computes HHI, assigns a risk rating, and displays the leading producer countries.
...
That is why VISTA places mineral concentration next to other supply-chain signals. A critical HHI score plus an export-control headline plus a chokepoint disruption is a different risk profile than a critical HHI score alone.
...
VISTA computes the Critical Minerals view from a committed 2024 production dataset and transparent scoring logic. The HHI formula is public in the codebase, and the risk thresholds are simple enough for a spreadsheet check.
