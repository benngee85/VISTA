...
import {
  parseGldArchiveRows,
  computeFlows,
} from '../scripts/seed-gold-etf-flows.mjs';

function buildSyntheticRows(
  rows,
  sheetName = 'US GLD Historical Archive',
) {
  void sheetName;
...
function buildHistoricalRows(n, { tonnesBase = 900, aumBase = 90e9, navBase = 78 } = {}) {
  const rows = [];
  for (let i = n - 1; i >= 0; i--) {
    const iso = daysAgoIso(i);
    rows.push({ date: spdrDate(iso), nav: navBase + i * 0.01, tonnes: tonnesBase + i, aum: aumBase + i * 1e6 });
  }
  return buildSyntheticRows(rows);
}

describe('seed-gold-etf-flows: parseGldArchiveRows', () => {
  it('parses the real SPDR column layout (Date=1, Close=2, Tonnes=10, AUM=11)', async () => {
    const sourceRows = buildHistoricalRows(15);
    const rows = parseGldArchiveRows(sourceRows);
  ...
  it('accepts "DD-MMM-YYYY" dates (real SPDR format)', async () => {
    const sourceRows = buildSyntheticRows(
  ...
  it('skips rows with zero or negative tonnage', async () => {
    ...
    const sourceRows = buildSyntheticRows([...good, ...bad]);
    const rows = parseGldArchiveRows(sourceRows);
