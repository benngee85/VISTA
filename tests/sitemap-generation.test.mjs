...
describe('root sitemap generator', () => {
  ...
  it('normalizes positive-offset commit timestamps to their UTC calendar date', () => {
    const tempRoot = mkdtempSync(join(tmpdir(), 'wm-sitemap-timezone-'));
    try {
      execFileSync('git', ['init', '--initial-branch=main'], { cwd: tempRoot });
      ...
      writeFileSync(join(tempRoot, 'material.txt'), 'timezone-sensitive material\n');
      execFileSync('git', ['add', 'material.txt'], { cwd: tempRoot });
      execFileSync('git', ['commit', '-m', 'add timezone-sensitive material'], {
        cwd: tempRoot,
        env: {
          ...process.env,
          GIT_AUTHOR_DATE: '2026-07-28T00:15:00+10:00',
          GIT_COMMITTER_DATE: '2026-07-28T00:15:00+10:00',
        },
      });
